...
class BitInterface_train(object):
    ...
    def __init__(self, mongo_in, init_BTC, index_start=0, index_end=40000, step=100, history=100, fee=0.0025,
        ...
        #        for ii in range(self.index_start, self.index_end):
        #            self.price_list_total.append(mongoCol.find_one({"index":ii})['marketsum']['result'][0]['Last'])
...
class BitInterfaceNoBook(object):

    # A slightly faster version of BitInterface_train, does not load order books
    # Probably could integrate these two objects together
    ...
    def get_history(self):
        if self.timesgn > 0:
            ...
            return self.price_list_total[self.index - 1 + self.history:self.index - 1:-1]

    def clear_orders(self):
        return

    def reset(self, buff=1000):
        # Internal index. Used for syncing up with MongoDB
        # Resets to a random place in the dataset. The end has a buffer so as not to run into the
        # end of the data set while training

        tot_length = self.index_end - self.index_start
        buff_len = tot_length - 2 * buff

        # Set new index as some random time, inbetween buffer
        self.index = int(buff_len * np.random.rand() + buff)

        # Mongo collection that has gathered data
        self.BTC = self.init_BTC
        self.coin_quant = 0  # Current ammount of alt-coin purchased

        # Set time direction, for class balancing
        if self.time_rev == True:
            self.timesgn = int(np.sign(np.random.rand() * 2 - 1))
